            ...
            <div class="preview-options">
                ...
                <form method="POST" action="{{ route('carrinho.adicionarCarrinho') }}" class="form-group">
                    ...
                    <div class="input-group-append" style="padding-top: 15px">
                        <button type="submit" class="btn btn-outline-secondary catalogo" name="ok">Adicionar ao
                            Carrinho</button>
                    </div>
                </form>
            </div>
        ...
        <form method="GET" action="{{ route('catalogo.index') }}" class="form-group">
            ...
            <select class="custom-select" name="categoria" id="inputCategoria" aria-label="Categoria">
                ...
                @foreach ($categorias as $id => $nome)
                    <option value={{ $id }} {{ $id == $categoria ? 'selected' : '' }}>{{ $nome }}
                    </option>
                @endforeach
            </select>
                    ...
                    <div class="col-md-4">
                        <div class="card" style="width: 18rem;">
                            <div class="card-header d-flex justify-content-center">
                                <img id="{{ $estampa->id }}"
                                    src="{{ $estampa->cliente_id ? route('imagemEstampa', $estampa) : asset('storage/estampas/' . $estampa->imagem_url) }}"
                                    class="card-img-top estampa-img" alt="{{ $estampa->nome }}">
                            </div>
                            <div class="card-body">
                                <h5 class="card-title">{{ $estampa->nome }}</h5>
                                <p class="card-text">{{ $estampa->descricao }}</p>
                                <button href="" class="btn mr-2 center catalogo" onclick="on({{ $estampa->id }})">Check
                                    offer</button>

                                @if ($estampa->cliente)
                                    <a href="{{ route('admin.catalogo.estampas.edit', $estampa) }}"
                                        class="btn btn-warning btn-sm" role="button" aria-pressed="true">
                                        <i class="fas fa-pen"></i>
                                    </a>

                                    <form class="d-inline"
                                        action="{{ route('admin.catalogo.estampas.destroy', $estampa) }}" method="POST">
                                        @csrf
                                        @method("DELETE")
